...
class XacmlRequestMapperTest {
    ...
    void testMapperMedTokenX() {
        System.clearProperty("bruk.tokenx.token");
        ...
        assertThat(request.Request().Environment().Attribute()).isNotNull();
        assertThat(request.Request().Environment().Attribute()).hasSize(1);
        assertThat(request.Request().Environment().Attribute().get(0).AttributeId()).isEqualTo(AbacAttributtNøkkel.ENVIRONMENT_PEP_ID);
        assertThat(request.Request().Environment().Attribute().get(0).Value()).isEqualTo("local-app");

        validerBasisResourceAttributter(request, 1, 2);

        validerBasisSubjectAttributter(request, 3);
        assertThat(request.Request().AccessSubject().Attribute().get(2).AttributeId()).isEqualTo(AbacAttributtNøkkel.SUBJECT_LEVEL);
        assertThat(request.Request().AccessSubject().Attribute().get(2).Value()).isEqualTo(3);
    }
    ...
    private void validerResourceAttributter(final Set<String> expectedValues, final XacmlRequest request, String attributtKey) {
        var resultatVerdier = request.Request().Resource()
            .stream()
            .flatMap(it -> it.Attribute().stream()
    ...
    private void validerAleneomsorg(final Boolean expected, final XacmlRequest request) {
        var resultatVerdier = request.Request().Resource()
            .stream()
            .flatMap(it -> it.Attribute().stream()
                .filter(s -> s.AttributeId().equals(AbacAttributtNøkkel.RESOURCE_FORELDREPENGER_ALENEOMSORG))
                .map(XacmlRequest.Pair::Value))
            .collect(Collectors.toSet());
    ...
    private void validerBasisSubjectAttributter(final XacmlRequest request, int antall) {
        ...
        assertThat(request.Request().AccessSubject().Attribute()).isNotNull();
        assertThat(request.Request().AccessSubject().Attribute()).hasSize(antall);
        assertThat(request.Request().AccessSubject().Attribute().get(0).AttributeId()).isEqualTo(AbacAttributtNøkkel.SUBJECT_ID);
        assertThat(request.Request().AccessSubject().Attribute().get(0).Value()).isEqualTo("srvTest");
        assertThat(request.Request().AccessSubject().Attribute().get(1).AttributeId()).isEqualTo(AbacAttributtNøkkel.SUBJECT_TYPE);
        assertThat(request.Request().AccessSubject().Attribute().get(1).Value()).isEqualTo("EksternBruker");
    }
    ...
    private void validerBasisResourceAttributter(final XacmlRequest request, int antallRessources, int antallAttributterPerRessource) {
        ...
        assertThat(request.Request().Resource().get(0).Attribute()).isNotNull();
        assertThat(request.Request().Resource().get(0).Attribute()).hasSize(antallAttributterPerRessource);
        assertThat(request.Request().Resource().get(0).Attribute().get(0).AttributeId()).isEqualTo(AbacAttributtNøkkel.RESOURCE_DOMENE);
        assertThat(request.Request().Resource().get(0).Attribute().get(0).Value()).isEqualTo("foreldre");
        assertThat(request.Request().Resource().get(0).Attribute().get(1).AttributeId()).isEqualTo(AbacAttributtNøkkel.RESOURCE_RESOURCE_TYPE);
        assertThat(request.Request().Resource().get(0).Attribute().get(1).Value()).isEqualTo("no.nav.abac.attributter.foreldrepenger.fagsak");
    }

    private void validerBasisEnvironmentAttributter(final XacmlRequest request) {
        validerBasisEnvironmentAttributter(request, "local-app", JWT_TOKEN.split("\\.")[1], AbacAttributtNøkkel.ENVIRONMENT_OIDC_TOKEN_BODY);
    }
    ...
    private void validerBasisEnvironmentAttributter(final XacmlRequest request, String expectedPepId, String expectedToken, String expectedTokenKey) {
        assertThat(request.Request().Environment()).isNotNull();
        assertThat(request.Request().Environment().Attribute()).isNotNull();
        assertThat(request.Request().Environment().Attribute()).hasSize(2);
        assertThat(request.Request().Environment().Attribute().get(0).AttributeId()).isEqualTo(AbacAttributtNøkkel.ENVIRONMENT_PEP_ID);
        assertThat(request.Request().Environment().Attribute().get(0).Value()).isEqualTo(expectedPepId);
        assertThat(request.Request().Environment().Attribute().get(1).AttributeId()).isEqualTo(expectedTokenKey);
        assertThat(request.Request().Environment().Attribute().get(1).Value()).isEqualTo(expectedToken);
    }

    private void validerBasisActionAttributter(final XacmlRequest request) {
        assertThat(request.Request()).isNotNull();
        assertThat(request.Request().Action()).isNotNull();
        assertThat(request.Request().Action().Attribute()).isNotNull();
        assertThat(request.Request().Action().Attribute()).hasSize(1);
        assertThat(request.Request().Action().Attribute().get(0).AttributeId()).isEqualTo(AbacAttributtNøkkel.ACTION_ACTION_ID);
        assertThat(request.Request().Action().Attribute().get(0).Value()).isEqualTo("read");
    }

...
public class NyXacmlRequestMapper {
    ...
    public static XacmlRequest lagXacmlRequest(PdpRequest pdpRequest) {
        ...
        var medToken = ENV.getProperty("bruk.tokenx.token", Boolean.class);
        if (medToken) {
        ...
        var resourceAttributes = ResourceInfo(pdpRequest);

        var request = new XacmlRequest.Request(actionAttributes, envAttributes, resourceAttributes, subjectAttributes.Attribute().isEmpty() ? null : subjectAttributes);
        return new XacmlRequest(request);
    }

    private static List<XacmlRequest.AttributeSet> ResourceInfo(final PdpRequest pdpRequest) {
        var resourceAttributes = new ArrayList<XacmlRequest.AttributeSet>();
        var identer = hentIdenter(pdpRequest);
        if (identer.isEmpty()) {
            resourceAttributes.addAll(ResourceInfo(pdpRequest, null));
        } else {
            identer.forEach(ident -> resourceAttributes.addAll(ResourceInfo(pdpRequest, ident)));
        }
        return resourceAttributes;
    }

    private static List<XacmlRequest.AttributeSet> ResourceInfo(final PdpRequest pdpRequest, Ident ident) {
        var resourceAttributes = new ArrayList<XacmlRequest.AttributeSet>();
        var aksjonspunktTyper = pdpRequest.getAksjonspunkter();
        if (aksjonspunktTyper.isEmpty()) {
            resourceAttributes.add(ResourceInfo(pdpRequest, ident, null));
        } else {
            aksjonspunktTyper.forEach(ap -> resourceAttributes.add(ResourceInfo(pdpRequest, ident, ap)));
        }
        return resourceAttributes;
    }

    private static XacmlRequest.AttributeSet ResourceInfo(PdpRequest pdpRequest, Ident ident, String aksjonsounktType) {
